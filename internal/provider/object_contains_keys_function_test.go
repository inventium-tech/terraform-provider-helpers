...
func TestObjectContainsKeysFunctionNonObjectInput(t *testing.T) {
	t.Parallel()
	...
	resource.UnitTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: `
					output "contains_keys_non_object" {
						value = provider::helpers::object_contains_keys(["item1", "item2"], toset(["key"]))
					}
				`,
